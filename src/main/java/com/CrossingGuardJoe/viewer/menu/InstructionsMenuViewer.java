...
public class InstructionsMenuViewer extends Viewer<InstructionsMenu> implements MenuInformationDrawer {
    ...
    private void drawPageFour(GUI gui) {
        ...
        int HP_BAR_X = 230;
        gui.drawImage(new Position(199, HP_BAR_X - 1), Shape.RectangleFilledGenerator(202, 39, ' ', 2, '$'));
        gui.drawImage(new Position(200, HP_BAR_X), HP_BAR);

        int HP_iniX = 208;
        for (int i = 0; i < 3; i++) {
            gui.drawImage(new Position(HP_iniX, HP_BAR_X + 4), HUDImages.MINI_KID);
            HP_iniX += 25;
        }
    }
